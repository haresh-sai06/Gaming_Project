...
function CustomCursor({ isHovering3D }){
    ...
    return (
        ...
        >
            ...
            {isHovering3D && (
                <motion.div className='absolute inset-0 rounded-full bg-transition border border-violet-500' initial= {{scale:0.5, opacity: 0 }} animate = {{scale:2 , opacity:0.5}} transition={{duration:1, repeat: Number.POSITIVE_INFINITY}} />
            )}
        </motion.div>
    )
}
...
const Characters = () => {
    ...
    const currentAvatar = Avatar[selectAvatar];
    ...
    <div className='relative w-full h-screen overflow-hidden mb-[10%]'>
        ...
        <div className='relative z-10 flex md:flex-row flex-col items-center w-full h-full p-4'>
            <div className='w-full md:w-2/4 flex flex-col md:ml-10'>
                <div className='bg-gray-900/80 backdrop-blur-lg rounded-lg p-4 mb-4 border-gray-800 shadow-[0_0_15px_rgba(167,139,250,0.2)]'>
                ...
                <div className='grid grid-cols-2 gap-4'>
                    <div className='relative bg-gray-900/70 backdrop-blur-sm rounded-lg p-3 border flex lg:flex-row flex-col justify-between px-12 items-center cursor-pointer transition-all duration-300' onClick={() => setSelectAvatar("VIKI")}>
                        <div className='text-lg mb-2'>
                            ...
                            <div className='flex'>
                                {[...Array(3)].map((_, index) => (
                                    <Star key={index} className={`h-4 w-4 fill-violet-400 text-violet-500`} />
                                ))}
                            </div>
                            {selectAvatar === "VIKI" && (
                    ...
                    <div className='relative bg-gray-900/70 backdrop-blur-sm rounded-lg p-3 border flex lg:flex-row flex-col justify-between px-12 items-center cursor-pointer transition-all duration-300' onClick={() => setSelectAvatar("EVA")}>
                        <div className='text-lg mb-2'>
                            EVA
                            <div className='w-20 h-20 bg-gray-800/50 rounded-md flex items-center justify-center mb-2'>
            ...
            {/*3d Model Section*/}
            <div className='relative md:w-2/4 2-full md:h-full h-80 flex items-center justify-center overflow-hidden' onMouseEnter={handle3DAreaMouseEnter} onMouseLeave={handle3DAreaMouseLeave}>
                ...
                {selectAvatar === "VIKI" ? (
                    ...
                    >
                        <Spline scene="https://prod.spline.design/rnPjv1UjcBeCUZcE/scene.splinecode" />
                    </motion.div>
                ) : (
                    <motion.div
                        key="EVA"
                        ...
                        transition={{duration: 0.5, ease: "easeInOut"}}
                    >
                        <Spline scene="https://prod.spline.design/OKA5JVzT-b7-rZkW/scene.splinecode" />
        </motion.div>
                )}
                </AnimatePresence>
            </div>
